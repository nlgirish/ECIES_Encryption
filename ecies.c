...
void * ecies_key_derivation(const void *input, size_t ilen, void *output, size_t *olen)
{
    ...
    *olen = SHA256_DIGEST_LENGTH;
    return SHA256(input, ilen, output);
}
...
secure_t * ecies_encrypt(char *key, unsigned char *data, size_t length, int *clen)
{
    ...
    size_t envelope_length, key_length;
    unsigned char envelope_key[SHA256_DIGEST_LENGTH], iv[EVP_MAX_IV_LENGTH];
    ...
    if ((key_length = EVP_CIPHER_key_length(ECIES_CIPHER)) > SHA256_DIGEST_LENGTH)
    {
    ...
    else if (ECDH_compute_key(envelope_key, SHA256_DIGEST_LENGTH, EC_KEY_get0_public_key(user), ephemeral, ecies_key_derivation) != SHA256_DIGEST_LENGTH)
    {
    ...
    else if ((bignum = (BIGNUM*)EC_POINT_point2bn(EC_KEY_get0_group(ephemeral), EC_KEY_get0_public_key(ephemeral), POINT_CONVERSION_COMPRESSED, NULL, NULL)) == NULL)
    {
    ...
    else if((envelope_length = bignum->top) == 0)
    {
        printf("Invalid envelope length. {envelope = %zu}\n", envelope_length);
        EC_KEY_free(ephemeral);
        EC_KEY_free(user);
        return NULL;
    }
    ...
    else if ((block_length = EVP_CIPHER_block_size(ECIES_CIPHER)) == 0 || block_length > EVP_MAX_BLOCK_LENGTH || (envelope_length = bignum->top) == 0)
    {
    ...
    else if (!(cryptex = secure_alloc(envelope_length, clen)))
    {
    ...
    memcpy(secure_key_data(cryptex), bignum->d, (bignum->top * sizeof(unsigned long)));
    ...
    if (EVP_EncryptInit_ex(&cipher, ECIES_CIPHER, NULL, envelope_key, iv) != 1 || EVP_CIPHER_CTX_set_padding(&cipher, 0) != 1 || EVP_EncryptUpdate(&cipher, body, &body_length, data, length) != 1)
    {
    ...
    {
        printf("ERROR!! Encryption created a wrong length Data\n");
        EVP_CIPHER_CTX_cleanup(&cipher);
        secure_free(cryptex);
        return NULL;
#if 0
    ...
    if (HMAC_Init_ex(&hmac, envelope_key, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(&hmac, secure_body_data(cryptex), secure_body_length(cryptex)) != 1 || HMAC_Final(&hmac, secure_mac_data(cryptex), &mac_length) != 1)
    {
...
unsigned char * ecies_decrypt(char *key, secure_t *cryptex, size_t *length)
{
    ...
    unsigned char envelope_key[SHA256_DIGEST_LENGTH], iv[EVP_MAX_IV_LENGTH], md[EVP_MAX_MD_SIZE], *block, *output;
    ...
    // Make sure we are generating enough key material for the symmetric ciphers.
    else if ((key_length = EVP_CIPHER_key_length(ECIES_CIPHER)) > SHA256_DIGEST_LENGTH)
    {
        printf("The key derivation method will not produce enough envelope key material for the chosen ciphers. {envelope = %i / required = %zu}", SHA256_DIGEST_LENGTH / 8, (key_length * 2) / 8);
        return NULL;
    }
    ...
    // Use the intersection of the provided keys to generate the envelope data used by the ciphers below. The ecies_key_derivation() function uses
    // SHA 256 to ensure we have a sufficient amount of envelope key material and that the material created is sufficiently secure.
    else if (ECDH_compute_key(envelope_key, SHA256_DIGEST_LENGTH, EC_KEY_get0_public_key(ephemeral), user, ecies_key_derivation) != SHA256_DIGEST_LENGTH)
    {
        printf("An error occurred while trying to compute the envelope key. {error = %s}\n", ERR_error_string(ERR_get_error(), NULL));
        EC_KEY_free(ephemeral);
        EC_KEY_free(user);
    ...
    // At the moment we are generating the hash using encrypted data. At some point we may want to validate the original text instead.
    if (HMAC_Init_ex(&hmac, envelope_key, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(&hmac, secure_body_data(cryptex), secure_body_length(cryptex)) != 1 || HMAC_Final(&hmac, md, &mac_length) != 1)
    {
    ...
    *length = secure_body_length(cryptex);
    return output;
}
