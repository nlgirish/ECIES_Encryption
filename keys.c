...
EC_KEY * ecies_key_create_public_octets(unsigned char *octets, size_t length)
{
    ...
    BIGNUM *tmp_bn = NULL;
    tmp_bn = (BIGNUM*)malloc(sizeof(BIGNUM));
    memset(tmp_bn, 0, sizeof(BIGNUM));
    tmp_bn->d = (BN_ULONG*)malloc(length  * sizeof(unsigned long));
    memset(tmp_bn->d, 0, length * sizeof(unsigned long));

    //Populate BIGNUM Data structure
    tmp_bn->top = length;
    memcpy(tmp_bn->d, octets, length * sizeof(unsigned long));


    if (EC_POINT_bn2point(group, tmp_bn, point, NULL) == NULL)
    {
